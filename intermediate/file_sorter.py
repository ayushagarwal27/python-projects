...
def sort_files(srcPath: str):
    """Sort files based on a given path"""

    for root_dir, sub_dir, filenames in os.walk(srcPath):
        for filename in filenames:
            file_path: str = os.path.join(root_dir, filename)
            extension: str = os.path.splitext(filename)[1]

            if extension:
                target_folder: str = create_folder(srcPath, extension)
                target_path: str = os.path.join(target_folder, filename)

                shutil.move(file_path, target_path)


def remove_empty_folders(srcPath: str):
    """Removes all empty folders"""

    for root_dir, sub_dir, filenames in os.walk(srcPath, topdown=False):
